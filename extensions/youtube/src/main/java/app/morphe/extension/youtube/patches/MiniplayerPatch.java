...
public final class MiniplayerPatch {
    ...
    /**
     * Legacy miniplayer preset (classic 14.x rectangle): when enabled, forces the modern
     * miniplayer flags to a fixed bottom-right square ~180dp with swipe-up=maximize and
    ...
    // Legacy hides the overlay buttons in LegacyMiniplayerController (setVisibility, not removal)
    // so they remain in the hierarchy and can still be triggered for maximize/dismiss.
    ...
    public static boolean getMiniplayerDoubleTapAction(boolean original) {
        // Legacy: force oxc.a() FALSE (double-tap + drag&drop both off) so YouTube does NO
        // gesture handling on the miniplayer — no tap-maximize, no double-tap-zoom, no pinch.
        // Our own gesture layer (LegacyMiniplayerController) provides swipe-up=maximize and
        // swipe-left=dismiss instead.
    ...
    public static boolean getMiniplayerDragAndDrop(boolean original) {
        if (LEGACY) return false; // oxb.i() then hard-forces BOTTOM_RIGHT (fixed corner)
        if (CURRENT_TYPE == DEFAULT) {
            return original;
        }
    ...
    public static boolean getHorizontalDrag(boolean original) {
        // Legacy: FALSE so YouTube's native offscreen-peek (oxj) never engages. Our snapshot
        // overlay + own gesture layer provide the 14.x swipe-to-dismiss instead.
        if (LEGACY) return false;
        if (CURRENT_TYPE == DEFAULT) {
            return original;
        }
    ...
    public static int getMiniplayerDefaultSize(int original) {
        if (LEGACY) return LEGACY_WIDTH_DIP; // ~180dp classic size
